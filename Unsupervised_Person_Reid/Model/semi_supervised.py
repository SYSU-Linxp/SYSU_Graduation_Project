...
LIST = os.path.join(DATASET, '50train.list')
...
for ckpt in range(START, END+1):
  ...
  sys.stdout.flush()
  images = np.array(labeled_images + [unlabeled_images[i][0] for i in reliable_image_idx])
  labels = to_categorical(known_labels + [kmeans.labels_[i] + KNOWN for i in reliable_image_idx])
  ...
  x = Dropout(0.5)(x)
  x = Dense(NUM_IDS, activation='softmax', name='fc8', kernel_initializer=RandomNormal(mean=0.0, stddev=0.001))(x)
