...
for ckpt in range(START, END+1):
  ...
  cluster_res = kmeans.predict(features)
  ...
  # rerank every cluster
  query_feature = centers
  gallery_feature = features
  q_g_dist = np.dot(query_feature, np.transpose(gallery_feature))
  q_q_dist = np.dot(query_feature, np.transpose(query_feature))
  g_g_dist = np.dot(gallery_feature, np.transpose(gallery_feature))
  re_rank = re_ranking(q_g_dist, q_q_dist, g_g_dist)

  re_features = features
  cluster_idx = []
  for i in range(len(center_idx)):
    index = np.argsort(re_rank[i,:])
    idx = kmeans.predict(features[center_idx[i]])
    cluster_idx = np.argwhere(cluster_res == idx)
    for j in range(len(cluster_idx)):
      re_features[cluster_idx[j]] = features[index[j]]
  
  # calculate similarity matrix
  similarities = sess.run(similarity, {center_t: centers, other_t: re_features}) # NUM_CLUSTER * num images
